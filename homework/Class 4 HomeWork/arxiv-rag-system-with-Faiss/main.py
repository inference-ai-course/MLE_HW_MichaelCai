...
def load_faiss_service()-> tuple[FaissService, OpenAIEmbeddingService]:
    try:
        ...
        if existing_chunks:
            ...
            if not unique_chunks:
                ...
                return (faiss_service, embedding_service)
...
def get_services():
    ...
    global faiss_service, embedding_service
    
    if faiss_service is None or embedding_service is None:
        try:
            print("Initializing Faiss and embedding services...")
            import sys
            sys.stdout.flush()  # Force output to show immediately
            
            faiss_service, embedding_service = load_faiss_service()
...
async def initialize_services():
    ...
    try:
        faiss_svc, embedding_svc = get_services()
        return {
            "status": "success", 
            "message": "Services initialized successfully",
            "documents": faiss_svc.get_stats()['total_documents']
        }
    except Exception as e:
...
async def search(q: str) -> dict:
    ...
    try:
        ...
        docs = faiss_svc.search(embedding_svc.embed_query(q))
        if not docs:
